...
// @version      7.3.4-beta
// @description  Beta version of RLM V7.3.4 with multi-language support and server-specific building IDs
...
(function() {
        ...
        if (origOnload && details.url && String(details.url).indexOf('/api/poi-types') !== -1) {
            details.onload = function(response) {
                try {
                    ...
                    console.warn('RLM V7 Beta Loader: could not inject RLM tags into poi-types', e);
                }
                origOnload(response);
            };
        }
    ...
    function loadBetaEntryPoint() {
        ...
        GM_xmlhttpRequest({
            method: 'GET',
            url: `https://realism-location-marker.com/api/beta-entry-point?_t=${timestamp}`,
            headers: headers,
